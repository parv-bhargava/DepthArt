...
def eval_submission(submission_csv_path, ground_truth_csv_path, rotation_thresholds_degrees_dict,
                    translation_thresholds_meters_dict, verbose=False):
...
def clean_and_convert(data_str):
    # Remove brackets and split by spaces
    cleaned = data_str.translate({ord(i): None for i in '[]\n'}).split()
    # Convert strings to float and reshape into matrix or vector
    return np.array(list(map(float, cleaned)), dtype=np.float32)
...
with open(f'{src}/train/train_labels.csv', 'r') as fr, open('submission.csv', 'w') as fw:
    for i, l in enumerate(fr):
        if i == 0:
            ...
            df = pd.read_csv('rot_tra_info.csv')
            # Apply the conversion function to 'Rotation Matrix' and 'Translation' columns
            df['Rotation Matrix'] = df['Rotation Matrix'].apply(lambda x: clean_and_convert(x).reshape(3, 3))
            df['Translation'] = df['Translation'].apply(lambda x: clean_and_convert(x).reshape(3))

            R = []
            T = []
            for index, row in df.iterrows():
                R = row['Rotation Matrix']
                T = row['Translation']
